...
void list_add_ascending_by_blocksize(list_t *l, block_t *newblk){
   ...
   */
    ...
    node_t *prev;
    node_t *newNode = node_alloc(newblk);
    int blocksize = newNode->blk->end - newNode->blk->start;
    int curr_blocksize;
    ...
    if(l->head == NULL){
      ...
      if(c->next == NULL) { 
         ...
         if(compareSize(blocksize,c)) {  
            ...
            while(c != NULL && blocksize <= curr_blocksize) {
                 prev = c;
                 c = c->next;

                 if(c != NULL){  
                       curr_blocksize = c->blk->end - c->blk->start;
                }
            }
            prev->next = newNode;
            newNode->next = c;
...
void list_coalese_nodes(list_t *l){ 
  ...
  while(curr){
    //if the prev.END + 1 == current.START
    if (prev->blk->end + 1 == curr->blk->start){
      // prev.END = current.END.
      prev->blk->end = curr->blk->end;
      // point the prev.NEXT to the current.NEXT to skip over current.
      prev->next = curr->next;
      //Free current
      free(curr->blk);
      free(curr);
...
bool list_is_in_by_pid(list_t *l, int pid){ 
  ...
  node_t *curr = l->head;
  while(curr != NULL){
    // find a node with a blk that has blk->pid = pid
